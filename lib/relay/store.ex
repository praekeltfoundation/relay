defmodule Relay.Store do
  ...
  def handle_call({:subscribe, xds, pid}, _from, state), do: subscribe_impl(xds, pid, state)

  def handle_call({:unsubscribe, xds, pid}, _from, state), do: unsubscribe_impl(xds, pid, state)

  def handle_call({:update, xds, version_info, resources}, _from, state),
    do: update_impl(xds, version_info, resources, state)
